...
begin
  ...
  try
    try
      ...
      if vPessoa = nil then
      begin
        TMessageUtil.Alerta('O cliente informado não existe. Verifique o código do cliente.');
        edtCodigo.Clear;
        edtNome.Clear;
        if edtCodigo.CanFocus then
           edtCodigo.SetFocus;
...
begin
  ...
  begin
    TMessageUtil.Alerta('O nome do cliente não pode ficar em branco.  ');
  ...
  begin
    TMessageUtil.Alerta('A forma de pagamento da venda não pode ficar em branco.  ');
  ...
  begin
    TMessageUtil.Alerta('O código do produto a ser vendido não pode ficar em branco.  ');
...
begin
  ...
  begin
    ...
    begin
      dbgProduto.DataSource.DataSet.Edit;
      dbgProduto.DataSource.DataSet.FieldByName('Preço Total').AsFloat := dbgProduto.DataSource.DataSet.FieldByName('Preço Uni.').AsFloat * dbgProduto.DataSource.DataSet.FieldByName('Quant.').AsFloat;
      dbgProduto.DataSource.DataSet.Post;
    end;

    if (dbgProduto.DataSource.DataSet.FieldByName('Descrição').AsString <> EmptyStr) and (dbgProduto.SelectedIndex = 4) then
    begin
...
begin
  try
    try
      ...
      xIDProduto := dbgProduto.DataSource.DataSet.FieldByName('Código').AsInteger;
      ...
      while not dbgProduto.DataSource.DataSet.Eof do
      begin
        if (dbgProduto.DataSource.DataSet.FieldByName('Código').AsInteger = xIDProduto) and (dbgProduto.DataSource.DataSet.FieldByName('Descrição').AsString <> EmptyStr) then
        begin
          dbgProduto.DataSource.DataSet.Edit;
...
begin
  try
    ...
    if (dbgProduto.DataSource.DataSet.FieldByName('Descrição').AsString <> EmptyStr) and (dbgProduto.SelectedIndex = 0) then
    begin
      dbgProduto.DataSource.DataSet.Append;
    end;
